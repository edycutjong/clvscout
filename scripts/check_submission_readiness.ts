 ...
 * targets): unpaid POST 402 shape, GET 405, free-endpoint 200s, plus the
...
async function main(): Promise<void> {
  ...
  const grade405 = await fetch(`${base}/api/grade`, { method: "GET" });
  if (grade405.status !== 405) failures.push(`GET /api/grade expected 405, got ${grade405.status}`);
  ...
  console.log("READINESS: PASS — 402 shape both paid routes, GET 405, free endpoints 200, truth-table sums OK");
}
